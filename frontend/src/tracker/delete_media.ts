...
const DELETE_MEDIA_API_BASE_URL = window.location.hostname.includes(
  "pythonanywhere.com"
)
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const titleH1 = document.getElementById("delete-title") as HTMLHeadingElement;
  ...
  deleteBtn.addEventListener("click", async () => {
    try {
      ...
      const response = await fetch(
        `${DELETE_MEDIA_API_BASE_URL}/api/media/${id}/`,
        {
          method: "DELETE",
          headers,
          credentials: "include",
